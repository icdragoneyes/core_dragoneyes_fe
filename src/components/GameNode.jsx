...
const GameNode = memo(({ node, animationValues }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <motion.div
        className="w-20 h-20 rounded-full bg-[#1E3557] bg-opacity-85 flex items-center justify-center cursor-pointer shadow-md hover:ring-4 hover:ring-[#F8B22A] hover:ring-opacity-78"
        whileHover={{ scale: 1.1, boxShadow: "0 0 15px 5px rgba(248, 178, 42, 0.78)" }}
      >
        <motion.img
          src={node.Icon}
          className="text-3xl text-white glitch-effect"
          alt={node.name}
      ...
      {/* card info */}
      <motion.div className="mt-4 bg-[#1E3557] bg-opacity-85 rounded-lg p-4 shadow-xl z-20 w-56" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
        <div className="relative w-full h-40 mb-4">
          <LazyLoadImage src={node.image} alt={node.name} className="w-full h-full object-cover rounded-lg" effect="blur" wrapperClassName="w-full h-full" />
